{
  Exports selected NPC records to NPC Visual JSON.
  Output: Data\Viny Mods\NPC Visual\NPC
}
unit ExportNPCVisualJSON;
...
begin
  ...
  edid := EditorID(e);
  if edid = '' then edid := IntToHex(FormID(e), 8);
  ...
  try
    ...
    saveDir := DataPath + 'Viny Mods\NPC Visual\NPC\';
    ForceDirectories(saveDir);
    filename := saveDir + edid + '.json';

    json.SaveToFile(filename);
    AddMessage('[NPC Visual] Exported NPC ' + edid + ' -> ' + filename);
  finally
    json.Free;
  end;
end;
